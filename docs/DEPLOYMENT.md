...
|---|---|---|
| `GOOGLE_SERVICES` | `build-mobile.yml` | **Preferred.** The entire contents of `google-services.json` as downloaded from Firebase console → Project settings → Your apps → the Android app registered for `com.HereLiesAz.BarBacker` (⚙️ next to that app → "Download google-services.json"), pasted whole into this one secret. `scripts/generate-google-services.js` writes it out as-is after checking its `package_name` matches `com.HereLiesAz.BarBacker` and its `mobilesdk_app_id` is an Android ID (`...:android:...`, not `...:web:...`) — this is the authoritative file Firebase itself generated, so there's no risk of the web app's ID leaking in by accident the way there is when reassembling it field-by-field. When this secret is set, the `VITE_FIREBASE_*`/`FIREBASE_ANDROID_*` fallback below is ignored entirely. |
| `VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_AUTH_DOMAIN`, `VITE_FIREBASE_PROJECT_ID`, `VITE_FIREBASE_STORAGE_BUCKET`, `VITE_FIREBASE_MESSAGING_SENDER_ID`, `VITE_FIREBASE_APP_ID`, `VITE_FIREBASE_VAPID_KEY` | `deploy.yml`, `build-mobile.yml` | Same Firebase Web SDK config used client-side (see `src/firebase.ts`) — injected as build-time env vars for both the web deploy and the Android web-asset build. Also doubles, along with `FIREBASE_ANDROID_APP_ID` below, as the fallback source for `google-services.json` when `GOOGLE_SERVICES` isn't set. |
| `FIREBASE_ANDROID_APP_ID` | `build-mobile.yml` | Fallback only — ignored when `GOOGLE_SERVICES` is set. The Firebase app ID of the *Android* app (`1:<project number>:android:<hash>`) — Firebase console → Project settings → Your apps → the app registered for `com.HereLiesAz.BarBacker` → "App ID". This is a **different app in the same project** than the web one `VITE_FIREBASE_APP_ID` holds (`...:web:...`), and the two are not interchangeable: `scripts/generate-google-services.js` writes it into `google-services.json` as `mobilesdk_app_id`, and the FCM backend rejects a token request whose app ID isn't bound to the requesting package. Every APK built before this secret existed shipped the web ID, which initializes Firebase fine and then fails every push registration. The generator refuses to write a file with a non-Android ID, so a missing/wrong value fails the build rather than shipping a push-dead APK. |
| `FIREBASE_ANDROID_API_KEY` | `build-mobile.yml` | Fallback only — ignored when `GOOGLE_SERVICES` is set. Falls back to `VITE_FIREBASE_API_KEY`. Set it when the browser key is restricted to HTTP referrers — such a key is rejected from the Android client. Firebase auto-creates an unrestricted "Android key" alongside the browser key. |
...
4.  **Resource Generation**:
    *   `scripts/generate-google-services.js` creates `google-services.json`
        from the `GOOGLE_SERVICES` secret if set (the whole file, verbatim),
        otherwise synthesizes it from the individual `VITE_FIREBASE_*` /
        `FIREBASE_ANDROID_APP_ID` secrets.
